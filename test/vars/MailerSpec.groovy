...
class MailerSpec extends JenkinsPipelineSpecification {
    ...
    def "[mailer.groovy] sendMarkdownTestSummaryNotification with no job id and no build url"() {
        setup:
        groovyScript.getBinding().setVariable('BUILD_URL', 'URL/')
        when:
        groovyScript.sendMarkdownTestSummaryNotification('SUBJECT', ['email@anything.com'])
        then:
        1 * getPipelineMock('util.getMarkdownTestSummary')('', '', 'URL/') >> 'CONTENT'
        1 * getPipelineMock('emailext')([ subject: 'SUBJECT', to: 'email@anything.com', body: 'CONTENT'])
    }

    def "[mailer.groovy] sendMarkdownTestSummaryNotification with no job id and build url"() {
        setup:
        groovyScript.getBinding().setVariable('BUILD_URL', 'URL/')
        when:
        groovyScript.sendMarkdownTestSummaryNotification('SUBJECT', ['email@anything.com'], '', 'BUILD_URL/')
        then:
        1 * getPipelineMock('util.getMarkdownTestSummary')('', '', 'BUILD_URL/') >> 'CONTENT'
        1 * getPipelineMock('emailext')([ subject: 'SUBJECT', to: 'email@anything.com', body: 'CONTENT'])
    }

    def "[mailer.groovy] sendMarkdownTestSummaryNotification with empty and no build url"() {
        setup:
        groovyScript.getBinding().setVariable('BUILD_URL', 'URL/')
        when:
        groovyScript.sendMarkdownTestSummaryNotification('', 'SUBJECT', ['email@anything.com'])
    ...
    def "[mailer.groovy] sendMarkdownTestSummaryNotification with no build url"() {
        ...
        then:
        1 * getPipelineMock('util.getMarkdownTestSummary')('', '', 'URL/') >> 'CONTENT'
        1 * getPipelineMock('emailext')([ subject: 'SUBJECT - JOB_ID', to: 'email@anything.com', body: 'CONTENT'])
    }
    ...
    def "[mailer.groovy] sendMarkdownTestSummaryNotification with multiple emails"() {
        ...
        then:
        1 * getPipelineMock('util.getMarkdownTestSummary')('', '', 'URL/') >> 'CONTENT'
        1 * getPipelineMock('emailext')([ subject: 'SUBJECT - JOB_ID', to: 'email@anything.com,email@otherthing.com', body: 'CONTENT'])
    }
    ...
    def "[mailer.groovy] sendMarkdownTestSummaryNotification with additional info"() {
        ...
        then:
        1 * getPipelineMock('util.getMarkdownTestSummary')('', 'ADDITIONAL_INFO', 'URL/') >> 'CONTENT'
        1 * getPipelineMock('emailext')([ subject: 'SUBJECT - JOB_ID', to: 'email@anything.com', body: 'CONTENT'])
    }
